      ...
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-8">
        ...
        <nav className="hidden items-center gap-1 sm:flex" aria-label="主导航">
          {navigation.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-muted-foreground transition hover:bg-white/[0.055] hover:text-foreground">
        ...
        <Link href="/sources" className="rounded-full border border-primary/30 bg-primary/8 px-3 py-1.5 text-[11px] font-medium text-primary">
          来源可核验
        </Link>
      </div>
    </header>
...
export function MobileNav() {
  return (
    <nav className="fixed inset-x-4 bottom-[max(1rem,env(safe-area-inset-bottom))] z-50 mx-auto grid max-w-md grid-cols-3 rounded-[22px] border border-white/10 bg-[#102027]/94 p-1.5 shadow-2xl backdrop-blur-xl sm:hidden" aria-label="移动端主导航">
      {navigation.map(({ href, label, icon: Icon }) => (
        <Link key={href} href={href} className="flex min-w-0 flex-col items-center gap-1 rounded-2xl px-2 py-2 text-[10px] font-medium text-muted-foreground transition active:bg-white/10 active:text-foreground">
          <Icon className="size-4" />
          <span>{label}</span>
        </Link>
      ))}
