...
export default function Home() {
  const officialCount = chips.reduce(
    (total, chip) => total + chip.officialSources.length,
    0,
  );
  return (
    <main className="mx-auto w-full max-w-7xl px-4 pt-7 sm:px-8 sm:pt-11">
        ...
        <div className="flex min-h-[430px] flex-col justify-between rounded-[32px] border border-white/8 bg-card/58 p-6 sm:p-9">
          <div>
            ...
            <h1 className="mt-6 max-w-3xl text-4xl font-semibold leading-[1.04] tracking-[-0.05em] sm:text-6xl">
              从 Memory 看懂
              <br />
              <span className="text-primary">AI 芯片</span>
            </h1>
            <p className="mt-5 max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">
              不只罗列 FLOPS。沿着 SRAM、HBM、LPDDR、DDR、HBF 与 scale-up
              互联，追踪每一种 AI 加速器如何搬运权重、激活和 KV Cache。
            </p>
          </div>
          ...
          <div className="mt-7 space-y-3">
            {[
              ...
              [
                ...
                '分层容量',
                'DDR · LPDDR · HBF · NVMe',
                '承接 KV、Embedding 与冷权重',
              ],
            ].map(([index, title, tech, body]) => (
              <div
                key={index}
                className="grid grid-cols-[auto_1fr] gap-3 rounded-2xl border border-white/8 bg-black/10 p-3.5"
              >
      ...
      <section className="grid grid-cols-2 gap-3 pt-5 sm:grid-cols-4">
        {[
          ...
          {
            value: String(memoryTopics.length),
            label: 'Memory 专题',
            icon: MemoryStick,
          },
      ...
      <section className="pt-12">
          ...
          {[
            {
              icon: Layers3,
              title: '参数不是排名',
              body: '保留芯片、板卡、系统与稠密/稀疏口径，不把不同边界的数字压成一个分数。',
            },
            {
              icon: BadgeCheck,
              title: '官方来源是硬门槛',
              body: '每个正式芯片条目至少链接一份厂商文档、官方技术博客或学术会议资料。',
            },
            {
              icon: BookOpenText,
              title: '第三方阅读有角色',
              body: '独立文章用于理解与质疑，不替代官方参数；推断会和已公开事实分开标注。',
            },
        ...
        <div className="mt-3 flex items-center justify-between rounded-[24px] border border-primary/16 bg-primary/[0.055] p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">可持续更新的数据层</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                官方源增量监测已配置；当前由 Codex 运行，接入 GitHub
                后每日自动执行。
              </p>
            </div>
          </div>
