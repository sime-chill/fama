    ...
    <main className="mx-auto w-full max-w-7xl px-4 pt-8 sm:px-8 sm:pt-12">
      <section className="max-w-3xl">
        ...
        <p className="mt-4 text-sm leading-7 text-muted-foreground">
          AI 系统不是只有“显存”。从片上 SRAM、封装内 HBM，到主机 DDR、低功耗
          LPDDR 与新兴 HBF，每一层都在容量、带宽、延迟、能效和成本之间交换。
        </p>
      </section>
        ...
        {memoryTopics.map((topic, index) => (
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-mono text-[10px] text-muted-foreground">
                  0{index + 1} · {topic.fullName}
                </p>
                <h2 className="mt-2 text-2xl font-semibold tracking-tight">
                  {topic.name}
                </h2>
              </div>
              <span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-white/[0.045]">
                <ArrowRight className="size-4 transition group-hover:translate-x-1 group-hover:text-primary" />
              </span>
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {topic.keyMetrics.slice(0, 3).map((metric) => (
                <span
                  key={metric.label}
                  className="rounded-full border border-white/8 bg-white/[0.035] px-3 py-1.5 text-[10px] text-muted-foreground"
                >
                  {metric.label} ·{' '}
            ...
            <p className="mt-5 text-[10px] font-medium text-primary">
              {topic.maturity}
            </p>
          </a>
        ))}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-7 text-muted-foreground">
              先问数据放在哪里，再问每秒搬多少、搬一次耗多少能量、谁管理搬移，最后才看计算峰值。这样才能把芯片规格连接到
              prefill、decode、训练和推荐等真实工作负载。
            </p>
          </div>
        </div>
      </section>
