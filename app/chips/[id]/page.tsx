    ...
    <main className="mx-auto w-full max-w-6xl px-4 pt-6 sm:px-8 sm:pt-9">
      ...
      <section className="mt-6 overflow-hidden rounded-[30px] border border-white/8 bg-card/64">
        <div
          ...
          style={{
            background: `linear-gradient(90deg, ${chip.accent}, transparent 72%)`,
          }}
        />
        <div className="grid gap-7 p-6 sm:p-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-primary/10 px-3 py-1.5 text-[10px] font-medium text-primary">
                {chip.vendor}
              </span>
              <span className="rounded-full border border-white/10 px-3 py-1.5 text-[10px] text-muted-foreground">
                {chip.category}
              </span>
              <span className="rounded-full border border-white/10 px-3 py-1.5 text-[10px] text-muted-foreground">
                {chip.status}
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 text-[10px]">
              <span className="inline-flex items-center gap-1.5 text-primary">
                ...
                {chip.officialSources.length} 个官方来源
              </span>
              <span className="text-muted-foreground">
                可信度 {chip.confidence}
              </span>
              <span className="text-muted-foreground">
                核验 {chip.lastVerified}
              </span>
            </div>
          </div>
      ...
      <section className="grid gap-8 pt-12 lg:grid-cols-[0.72fr_1.28fr]">
        ...
        <div className="space-y-2">
          {chip.memoryLayers.map((layer, index) => (
            <div
              key={layer.name}
              className="grid grid-cols-[auto_1fr] gap-4 rounded-[22px] border border-white/8 bg-card/52 p-4"
            >
              <span
                className="grid size-9 place-items-center rounded-xl font-mono text-xs font-semibold"
                style={{
                  background: `color-mix(in srgb, ${chip.accent} 15%, transparent)`,
                  color: chip.accent,
                }}
              >
              ...
              <div>
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <span className="text-[10px] text-primary">{layer.role}</span>
                </div>
        ...
        <aside className="space-y-3">
          <div className="rounded-[22px] border border-white/8 bg-white/[0.025] p-5">
            ...
            <ol className="mt-4 space-y-3">
              {chip.architecture.map((item, index) => (
                <li
                  key={item}
                  className="flex items-start gap-3 text-xs leading-5 text-muted-foreground"
                >
          ...
          <div className="rounded-[22px] border border-white/8 bg-white/[0.025] p-5">
            ...
            <dl className="mt-4 space-y-3 text-xs">
              <div>
                <dt className="text-muted-foreground">软件栈</dt>
                <dd className="mt-1 leading-5">{chip.software}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">目标工作负载</dt>
                <dd className="mt-1 leading-5">{chip.workload}</dd>
              </div>
            </dl>
          </div>
          <div className="rounded-[22px] border border-white/8 bg-white/[0.025] p-5">
      ...
      <section className="pt-12">
        <div>
          ...
          <h2 className="mt-1.5 text-2xl font-semibold">资料入口</h2>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          <div>
            ...
            <div className="space-y-2">
              {chip.officialSources.map((source) => (
          ...
          <div>
            ...
            <div className="space-y-2">
              {chip.thirdPartySources.map((source) => (
